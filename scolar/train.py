"""scOLAR training, fixed-final checkpointing, and dual-track evaluation.

The primary checkpoint is selected at a prespecified final epoch. Target labels
are excluded from optimization, calibration, and checkpoint selection, and are
read only during the post-training benchmark evaluation.
"""
...
SOURCE_ONLY_HVG_DATASETS = ("Cao", "Quake_10x", "Quake_Smart-seq2", "Wagner", "Zeisel_2018")
...
def _read_h5ad_fail_closed(path: Path):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read h5ad at '{absolute_path}': " f"{type(exc).__name__}: {exc}"
        ) from exc
...
def cluster_acc(y_pred: np.ndarray, y_true: np.ndarray) -> float:
    """Compute clustering accuracy after optimal Hungarian assignment."""
...
def aligned_cluster_acc_and_macro(y_true: np.ndarray, y_pred: np.ndarray, n_clusters: int):
    """Return Hungarian-aligned weighted and macro clustering accuracy."""
    y_true = y_true.astype(np.int64)
    y_pred = y_pred.astype(np.int64)
    D = max(y_pred.max(), y_true.max()) + 1
    w = np.zeros((D, D), dtype=np.int64)
...
def common_oracle_kmeans(
    ...
    weighted, macro, cm = aligned_cluster_acc_and_macro(y_true_dense, pred, n_clusters)
    return {
        "pred": pred,
...
def compute_joint_overall(
    ...
    """Compute one global Hungarian score over known predictions and novel clusters."""
...
class AverageMeter:
    """Track a running weighted mean for one scalar metric."""
...
BENCHMARK_CLASS_REGISTRY = {
    ...
    "Cao": {
        ...
        "n_cs": 6,
        "n_cr": 4,
        "n_ct": 6,  # Table S4: |Cs|=6, |Cr|=4, |Ct|=6
    },
    "Wagner": {
        "classes": [
        ...
        "n_cs": 5,
        "n_cr": 4,
        "n_ct": 5,  # Table S4: |Cs|=5, |Cr|=4, |Ct|=5
    },
    "Zeisel_2018": {
        "classes": [
        ...
        "n_cs": 6,
        "n_cr": 5,
        "n_ct": 6,  # Table S4: |Cs|=6, |Cr|=5, |Ct|=6
    },
}
...
def split_open_set_protocol(
    ...
    seed: int = 42,
    n_cs_override: int = None,
    n_cr_override: int = None,
):

    if dataset_name in BENCHMARK_CLASS_REGISTRY:
        entry = BENCHMARK_CLASS_REGISTRY[dataset_name]
        ...
        if missing:
            print(
                f"[Data] Warning: {len(missing)} classes in registry "
        ...
        print("=== [Open-Set Protocol] (fixed benchmark split) ===")
...
def _validate_source_only_hvg_manifest(args) -> dict:
    ...
    try:
        ...
        _source_only_manifest_error(f"cannot parse {manifest_path}: {type(exc).__name__}: {exc}")
    ...
    try:
        ...
        if (
            not freezer_path.is_file()
            or _sha256_file(freezer_path) != SOURCE_ONLY_HVG_EXPECTED_FREEZER_SHA256
        ):
        ...
        if not math.isclose(
            float(specification["pseudo_known_fraction"]),
            0.20,
            rel_tol=0.0,
            abs_tol=1e-12,
        ):
    ...
    role_hashes = {}
    for role in ("S", "K", "N"):
        role_ids[role], role_hashes[role] = validate_role(role, fold_payload["roles"][role])
    role_sets = {role: set(ids) for role, ids in role_ids.items()}
    if (
        role_sets["S"] & role_sets["K"]
        or role_sets["S"] & role_sets["N"]
    ...
    canonical_class_ids = {
        str(name): str(class_id) for name, class_id in class_registry["canonical_class_ids"].items()
    }
...
def prepare_source_only_hvg_episode(args, ontology_manager: OntologyManager):
    ...
    labels_source = np.asarray([str(value) for value in labels_all.iloc[source_rows]], dtype=object)
    ...
    for class_name in context["fold_payload"]["retained_class_names"]:
        ...
        class_k = _utf8_lexical(
            value for value in context["role_ids"]["K"] if source_id_to_label[value] == class_name
        )
        class_s = _utf8_lexical(
            value for value in context["role_ids"]["S"] if source_id_to_label[value] == class_name
        )
    ...
    episode_id_to_row = {stable_id: index for index, stable_id in enumerate(context["source_ids"])}
    role_rows = {
        role: np.asarray(
    ...
    if len(target_train_ds.tensors) != 3:
        raise RuntimeError(
            "source_only_hvg target training loader must contain exactly " "(x, raw_x, size_factor)"
        )
...
def prepare_datasets_robust(args, ontology_manager: OntologyManager):
    """Load and preprocess one intra-dataset benchmark under the fixed open-set split."""
    ...
    # Fixed intra-dataset benchmark protocol:
    ...
    print(
        f"[Data] Benchmark-exclusive split: source={len(src_idx)} target={len(tgt_idx)} "
    ...
    if novel_ratio < 1.0 - 1e-6:
        ...
        tgt_eval_valid_mask = src_valid_mask[tgt_idx]
        print(
            f"[Data] Novel ratio={novel_ratio:.1f}: "
    ...
    print(
        f"[Data] Source: {len(src_ds):>6} cells | "
...
def compute_adv_loss(
    ...
    """Compute the source decision-level margin loss from fine and detached coarse similarities."""
...
def compute_target_adv_loss(
    ...
    """Compute the target decision-level objective using reference-calibrated novelty decisions."""
    ...
    * All neighbour mining, ontology weighting and q_novel weighting are detached.
    ...
    with torch.no_grad():
        ...
        if ontology_gate == "soft":
            ...
            src_prob = F.softmax(src_logits / max(float(ontology_temperature), 1e-6), dim=1)
            ...
            if depth_mask.any():
                ...
                relation = torch.where(
                    ...
                    torch.eye(known_ids.numel(), device=z.device, dtype=anc.dtype),
                ).clamp(0.0, 1.0)
        ...
        if alpha is None:
            ...
            q_novel = torch.sigmoid((float(alpha) - mls) * float(confidence_temperature))
            if confidence_gate:
    ...
    with torch.no_grad():
        ...
        stats = {
            ...
            "safe_neg_frac": (
                float(has_safe_neg.float().mean().item()) if alpha is not None else 0.0
            ),
...
def compute_novel_sep_loss(
    ...
    novel_w = torch.sigmoid((alpha - mls) * temp).detach()
    if novel_w.sum() < 1e-6:
        return z_tgt.new_tensor(0.0, requires_grad=True)
...
def build_calibration_plan(
    ...
    *,
    ...
    """Create deterministic source-only pseudo-novel calibration draws."""
    all_targets = []
    ...
    for _ in range(int(n_simulations)):
        ...
        heldout_class_ids.append([int(v) for v in classes[perm[:n_to_mask]].tolist()])
...
def calibrate_threshold(
    ...
    *,
    ...
    """Estimate the maximum-logit novelty threshold from the fixed source-only calibration plan."""
    model.eval()
...
def lineage_consistency_check(
    ...
    novel_coarse_probs: torch.Tensor,
    ...
    """Aggregate source-class probabilities through ontology ancestry for post-hoc lineage context."""
    ...
    if cluster_labels is None:
        print(
            f"[LCC] Common Leiden clustering {z_np.shape[0]} predicted-novel cells "
        ...
        print(
            f"[LCC] Reusing Track-2 common Leiden partition: "
            f"{len(np.unique(clusters))} clusters."
        )

    depths_cpu = ontology_manager.depths.cpu()
    coarse_probs = novel_coarse_probs
    ...
    for cid in np.unique(clusters):
        ...
        if coverage >= coverage_thres:
            ...
            deepest_cl_id = (
                ontology_manager.all_terms[deepest_idx]
                if deepest_idx < len(ontology_manager.all_terms)
                else f"idx_{deepest_idx}"
            )
            ...
            for i in topk_indices:
                i = i.item()
                cl_id = (
                    ontology_manager.all_terms[i]
                    if i < len(ontology_manager.all_terms)
                    else f"idx_{i}"
                )
            ...
            label = (
                f"Potential subtype of <{deepest_name}> " f"(in {' / '.join(top_names)} lineage)"
            )
            status = "Consistent"
        else:
...
def _print_lcc_report(report: dict):
    """Print a compact lineage-consistency report."""
    if not report:
    ...
    print("\n" + "=" * 110)
    print(
        f"{'CID':<6} | {'Cells':>6} | {'Status':<12} | {'TopK-Cov':>9} | "
        f"{'Top1':>6} | Interpretation"
    )
    print("-" * 110)
    for cid, info in sorted(report.items()):
        print(
            f"{cid:<6} | {info['cells']:>6} | {info['status']:<12} | "
    ...
    print(
        f"[LCC Summary] Clusters={len(report)} | "
...
def validate_osr(
    ...
    with torch.no_grad():
        for x, _, _, y_idx, valid_mask, is_novel_gt, y_raw in tgt_loader:
            ...
            if novel_mask_b.any():
                all_novel_z_eval.append(z[novel_mask_b])
    ...
    if n_novel > 0 and n_known > 0:
        ...
        auroc = 0.5
        print(
            f"[Val] WARNING: AUROC=0.5 — GT Novel={n_novel}, GT Known={n_known}. "
    ...
    if all_novel_z_eval and novel_list and len(novel_list) > 1:
        ...
        print(
            f"  Frozen KMeans: seed={eval_kmeans_seed}, n_init={eval_kmeans_n_init}; "
        ...
        print(
            f"  ARI/AMI/NMI              : {track1['ari']:.4f}/{track1['ami']:.4f}/{track1['nmi']:.4f}"
        )
    ...
    if run_deployment_eval and novel_z_cache and n_pred_novel >= 10:
        ...
        tp_scores = _safe_cluster_scores(pred_true[pred_is_gt], deployment_clusters[pred_is_gt])
        track2 = {
        ...
        print(
            f"  PredNovel={n_pred_novel} | TP={tp} FP={fp} FN={fn} | "
            f"P/R/F1={precision:.4f}/{recall:.4f}/{f1:.4f}"
        )
        print(
            f"  Leiden clusters={track2['leiden_clusters']} | "
        ...
        eval_details.update(
            {
                ...
                "track1_oracle_kmeans": (
                    None
                    if track1 is None
                    else {k: v for k, v in track1.items() if k not in {"pred", "confusion"}}
                ),
    ...
    run_lcc = run_lcc_eval and ((epoch == "FINAL") or (isinstance(epoch, int) and epoch % 20 == 0))
            ...
            try:
                ...
                if report:
                    ...
                    for thres in [0.25, 0.30, 0.40, 0.50, 0.60, 0.70]:
                        n_consistent = sum(1 for v in report.values() if v["score"] >= thres)
                        print(
                            f"  coverage_thres={thres:.2f}: "
...
def _save_checkpoint(
    ...
    *,
    ...
    """Save model state, optimizer state, schedule metadata, and checkpoint provenance."""
    path = Path(path)
    ...
    print(f"[Checkpoint] Saved {role}: '{path}' " f"(epoch={epoch}, selection={selection_rule})")


def load_checkpoint(path, model, optimizer=None, scheduler=None, device="cpu"):
    """Load a model checkpoint and optionally restore optimizer and scheduler state."""
...
def _snapshot_checkpoint_path(final_path: str, epoch: int) -> str:
    """Derive a prespecified epoch-snapshot path from the final checkpoint path."""
...
def _default_snapshot_epochs(args) -> set[int]:
    ...
    if getattr(args, "cross_dataset", False):
        ...
        return {e for e in (args.epochs - 20, args.epochs - 10) if 1 <= e < args.epochs}
    ...
    return {
        e
        for e in range(args.first_calibrate_epoch, args.epochs, args.calibrate_every)
        if 1 <= e < args.epochs
    }
...
def _validate_protocol_configuration(args) -> None:
    ...
    if is_source_only:
        if args.dataset not in SOURCE_ONLY_HVG_DATASETS:
            raise ValueError(f"source_only_hvg --dataset must be one of {SOURCE_ONLY_HVG_DATASETS}")
        ...
        if args.hvg not in SOURCE_ONLY_HVG_ALLOWED_HVG:
            raise ValueError(f"source_only_hvg --hvg must be one of {SOURCE_ONLY_HVG_ALLOWED_HVG}")
        ...
        if args.posthoc_oracle_diagnostic:
            raise ValueError(
                "source_only_hvg forbids post-hoc target-selected checkpoint diagnostics"
            )
        ...
        args.source_only_manifest is not None
    ...
    if args.protocol_profile in {"paper_primary", "paper_schedule", "source_only_hvg"}:
        if not math.isclose(float(args.lambda_tnc), 0.0, rel_tol=0.0, abs_tol=1e-12):
            mismatches.append(f"lambda_tnc={args.lambda_tnc!r} (paper baseline requires 0.0)")
    ...
    if args.protocol_profile == "dev_tnc":
        ...
        if args.hcl_decay_start < args.epochs:
            mismatches.append(
                "Dev-B keeps HCL decay disabled; use Dev-C only after TNC is diagnosed"
            )
...
def train(args):
    ...
    probe_only = bool(
        args.protocol_profile == "source_only_hvg" and args.source_only_episode_probe_only
    )
    device = torch.device("cpu" if probe_only else ("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    if args.protocol_profile == "source_only_hvg":
        (
            ...
            le,
            episode_provenance,
        ) = prepare_source_only_hvg_episode(args, ontology)
        ...
        (
            ...
            novel_list,
            le,
        ) = prepare_cross_dataset(args, ontology)
    else:
        (
            src_train_loader,
            src_eval_loader,
            ...
            novel_list,
            le,
        ) = prepare_datasets_robust(args, ontology)
    ...
    def get_current_rec_weight(epoch: int) -> float:
        """Return the prespecified reconstruction-loss weight for an epoch."""
        ...
        progress = min(1.0, (epoch - args.rec_stop_epoch) / max(1, args.rec_rampdown_epochs))
    ...
    def get_current_hcl_weight(epoch: int) -> float:
        """Return the prespecified hierarchical-loss weight for an epoch."""
        if epoch < HCL_WARMUP:
            return 0.0
    ...
    calibration_plan_path = str(Path(final_ckpt_file).with_suffix(".calibration_draws.json"))
    Path(calibration_plan_path).parent.mkdir(parents=True, exist_ok=True)
    Path(calibration_plan_path).write_text(json.dumps(calibration_plan, indent=2), encoding="utf-8")
    ...
    print(
        "[Protocol] Target labels are unavailable to the training loader and are not used "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        should_calibrate = epoch == FIRST_CALIBRATE or (
            epoch > FIRST_CALIBRATE and (epoch - FIRST_CALIBRATE) % args.calibrate_every == 0
        )
        if should_calibrate:
        ...
        current_margin = get_hcl_margin(epoch)
        ...
        for batch_idx, (sx, sraw, ssf, sy, svalid) in enumerate(src_train_loader):
            ...
            if not (
                _finite(s_out["logits_fine"])
                and _finite(s_out["z"])
                and _finite(t_out["logits_fine"])
                and _finite(t_out["z"])
            ):
    ...
    if args.protocol_profile == "source_only_hvg":
        ...
        source_only_result_path = Path(final_ckpt_file).with_suffix(".source_only_hvg_result.json")
    ...
    if args.posthoc_oracle_diagnostic:
        ...
        for snapshot_epoch in candidate_epochs:
            ...
            values = validate_osr(
                ...
                ontology,
                unique_src_ids,
                epoch=f"POSTHOC_E{snapshot_epoch}",
                le=le,
                novel_list=novel_list,
                leiden_res=args.leiden_res,
                lcc_temperature=args.lcc_temperature,
        ...
        if diagnostic["checkpoints"]:
            ...
            diagnostic["oracle_target_selected"] = {
                ...
                **oracle_metrics,
                "optimism_gap_over_fixed_final_pp": 100.0
                * (oracle_metrics["overall_j"] - fixed["overall_j"]),
            }
            print(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="scOLAR Training", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    g_base.add_argument(
        "--output_dir", type=str, default="./checkpoints", help="Directory to save models and logs"
    )
    g_base.add_argument(
        "--protocol_profile",
        choices=[
    ...
    g_data.add_argument("--hvg", type=int, default=2000, help="Number of highly variable genes")
    g_data.add_argument(
        "--batch_size",
        type=int,
        default=1024,
    ...
    g_data.add_argument(
        ...
        "remain 100%% in source, following the fixed benchmark split protocol.",
    )
    g_data.add_argument(
        "--src_min_per_class",
        type=int,
    ...
    g_data.add_argument(
        "--source_only_fold",
        type=int,
        choices=list(SOURCE_ONLY_HVG_ALLOWED_FOLDS),
        default=None,
        help="Frozen pseudo-novel class fold for source_only_hvg",
    )
    g_data.add_argument(
        "--source_only_episode_probe_only",
        action="store_true",
    ...
    g_sched.add_argument("--lambda_hcl", type=float, default=0.1, help="Weight for source HCL")
    ...
    g_sched.add_argument(
        "--tnc_ontology_min_depth",
        type=int,
        default=2,
        help="Minimum ontology depth retained when computing ancestor-Jaccard compatibility",
    )

    g_sched.add_argument("--lambda_adv", type=float, default=0.05, help="Weight for L_dec-adv")

    g_sched.add_argument(
        "--lambda_novel_sep", type=float, default=0.01, help="Weight for novel separation loss"
    )
    g_sched.add_argument(
        "--novel_sep_temp",
        type=float,
    ...
    g_sched.add_argument("--rec_weight", type=float, default=1.0, help="Base reconstruction weight")
    g_sched.add_argument(
        "--rec_decay_rate",
        type=float,
        default=0.5,
    ...
    g_sched.add_argument("--hcl_warmup_epoch", type=int, default=20, help="Epoch to activate HCL")
    g_sched.add_argument(
        "--adv_warmup_epoch", type=int, default=50, help="Epoch to activate L_dec-adv"
    )
    ...
    g_sched.add_argument(
        "--hcl_decay_epochs", type=int, default=20, help="Epochs to ramp HCL from full to floor"
    )
    g_sched.add_argument(
        "--hcl_decay_floor",
        type=float,
    ...
    g_adv.add_argument(
        "--adv_margin", type=float, default=0.05, help="Slack in ReLU((beta - adv_margin) - gap)"
    )
    g_adv.add_argument("--adv_stop_epoch", type=int, default=999, help="Epoch to stop L_dec-adv")
    g_adv.add_argument(
        "--adv_mode",
        type=str,
        default="source",
    ...
    g_cross.add_argument(
        "--cross_dataset", action="store_true", help="run Frame-A cross-dataset open-set probe"
    )
    g_cross.add_argument(
        "--cross_h5ad", type=str, default="data/ALIGNED_Mus_musculus_Mammary_Gland.h5ad"
    )
    g_cross.add_argument("--cross_ref_name", type=str, default="Quake_Smart-seq2_Mammary_Gland")
    g_cross.add_argument("--cross_tgt_name", type=str, default="Quake_10x_Mammary_Gland")
    ...
    if not os.path.exists(args.ontology_path):
        print(
            f"[Error] Ontology not found at '{args.ontology_path}'. "
